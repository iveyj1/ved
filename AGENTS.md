...
**Read, bang, and ripgrep** — `:read <file>` inserts file contents below the cursor. `:read !<cmd>` inserts command output. `:! <cmd>` / `:!<cmd>` runs a shell command and shows one-line truncated output in the message bar. `:rg <pattern> [path]` runs ripgrep and captures output in the remembered quickfix buffer. `<space>o` parses `file:line:column:` under the cursor and opens/switches to that location. `_exec_read(arg)` handles reads; bang and rg commands use `subprocess.run`.
