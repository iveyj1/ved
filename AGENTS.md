...
- `test_vig.py` — PTY-based smoke tests (plain asserts, no framework, 200 test functions)
...
**Search** — `/` and `?` enter SEARCH mode, which captures a regex pattern in the command bar. On Enter, `_search_next(direction)` compiles the pattern with `re.compile` and searches from the position after/before the cursor, including additional hits on the current line before moving to other lines and wrapping around. Forward search uses `re.search`; backward search uses `re.finditer` to find the last match before the cursor. `n` repeats in the same direction; `N` reverses. The last pattern is stored in `search_pattern` and reused when Enter is pressed with an empty prompt.
